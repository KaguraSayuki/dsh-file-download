...
A file browser and download surface for the [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (DSH) Web GUI: browse the machine that serves the agent — including everything outside the session workspace — download a file, download a folder as a ZIP, and archive a multi-selection in one stream. It extends the official Sidebar through the official tab registry instead of patching DSH.
...
- **Authenticated, not anonymous.** Every route lives behind Connection's fence, exactly like `/api/remote.mux`. An unauthenticated visitor cannot list or fetch anything.
- **Read-only.** No upload, no write, no delete, no rename, no shell. The plugin exposes reads and archives only.
...
- **Bounded.** A listing is capped at 5000 entries, an archive at 20000 entries / 2 GiB / 64 MiB per file, and a multi-selection at 200 paths; over-cap work is refused rather than silently truncated.
...
- **No writes.** The browser cannot upload, rename, delete, or edit. Anything mutating belongs to the agent or to a different plugin.
