...
9. **Nothing buffers a whole entry.** A file download streams windows; an archive entry streams windows straight into its deflate stream and is closed by a data descriptor. Never reintroduce a whole-file buffer, and never let a ZIP offset leave the 32-bit fields this writer emits — stay under the total cap rather than half-implementing ZIP64.
10. **The ZIP method is measured, never assumed for unknown data.** A known-compressed extension is stored; anything else is probed with a level-1 deflate of its first window before the local header is written, because the method cannot change afterwards. Never deflate an unknown binary format blind, and never chase the ratio with a high level.
11. **Resume is a header, not a client.** The whole resumable-download feature is `Range` + `ETag` + `If-Range` on the file route; do not build a second transfer protocol in the browser. A generated archive advertises `Accept-Ranges: none` because it has no stable identity.
12. **The browse page ships no script.** No inline script, no external resource, no event attribute. It carries `default-src 'none'`, escapes every interpolated value, and never reflects a path into a URL that did not come from a listing entry.
13. **The browser half requires only platform seeds.** `react` and `react/jsx-runtime` are seeded by the shell's module table; anything else must not be `require`d. A missing module throws during boot and the whole GUI fails to load.
14. **The client `inject` list names real services.** Every name in `exports.inject` must be provided by the shipped client plane. A name that never appears leaves the plugin fiber pending, and the shell's boot assertion turns that into a failed page. Check `tests/contract.mjs` when adding one.
15. **DOM injection is append-only.** Append injected nodes at the end of their container. Never insert between React-managed siblings, never mutate React-owned attributes, and never remove or reorder official nodes. Every injected node carries `data-dsh-download`, and every effect removes what it added.
16. **Official-first.** Prefer an official slot, the tab registry, or an official service to a DOM decoration. A decoration must depend only on published `data-*` hooks, and the contract test must cover the hook it uses.
17. **No forks of official components.** If a surface needs a change that only a fork would allow, that is a signal to open an upstream issue or to add a smaller entry point, not to vendor a component.

## Layout

| Path | Role |
|---|---|
| `lib/index.js` | host half: constants, `resolveScope`, `resolveTarget`, `parseSandboxMode`/`sessionSandboxMode`/`modeGate`, `parseRange`/`etagOf`, `fsStatus`/`fsFailure`, `fileStream`, `listResponse`, `downloadResponse`, `browseResponse`/`renderBrowsePage`, the streaming ZIP writer (`crc32`, `zipMethodFor`, headers, `archiveChunks`, `streamFrom`, `archiveStream`), `archiveResponse`, `archiveSelectionResponse`, `uploadResponse`/`writeUpload`/`uploadFailure`, `apply` |
