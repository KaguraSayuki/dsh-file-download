...
`;
		...
		function BrowserBody(props) {
			...
			// names, then keep the names that start with what is being typed. It
			// reads `displayPath`, so it must sit after that declaration.
			React.useEffect(() => {
				if (!editingPath) {
					setSuggestions([]);
		...
		exports.BrowserTitle = BrowserTitle;
