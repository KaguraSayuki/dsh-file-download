...
/** Kept under the 32-bit ZIP offsets this writer emits, so no ZIP64 record is needed. */
const MAX_ARCHIVE_BYTES = 2 * 1024 * 1024 * 1024;
const MAX_ARCHIVE_SELECTION = 200;

/**
 * Deflate level for archive entries. This is zlib's ordinary default: a download
 * is not worth minutes of CPU for a few percent, and the methods that matter are
 * decided elsewhere — already-compressed formats and incompressible data are
 * stored rather than deflated at all.
 */
const ZIP_LEVEL = 6;
 ...
 */
export async function resolveTarget(ctx, scope, requested, signal) {
	const path = expandHome(requested === null || requested === undefined || requested === "" ? scope.workspaceRoot : requested, homedir());
	const target = await ctx.fs.resolve(path, { cwd: scope.workspaceRoot, signal });
	return { target, path: ctx.fs.processPath(target) };
}

/**
 * Expand a leading `~` to the host user's home directory. A browser cannot know
 * that path, so the shorthand is resolved where the filesystem lives.
 * @param value - the caller's path.
 * @param home - the host home directory.
 * @returns The path with `~` expanded; anything else is returned unchanged.
 */
 ...
 *
 * Only a single byte range is honoured; a multi-range or malformed header is
 * reported as "no range" so the caller serves the whole file, which is always a
 * valid answer. A syntactically valid but unsatisfiable range reports `null` so
 * the caller can answer 416.
 * @param header - the raw header value.
 * @param size - the complete entity size.
 * @returns `{ start, end }`, undefined for no range, or null when unsatisfiable.
 */
...
/**
 * The entity tag for one file: its version and size, which is exactly what a
 * resumed download has to agree on before the remainder may be appended.
 * @param path - the resolved path, so two files with one version cannot collide.
 * @param info - the stat result.
 * @returns A quoted strong validator.
 */
export function etagOf(path, info) {
	const version = String(info?.version ?? "0").replace(/[^\w.:-]/gu, "_");
	const size = typeof info?.size === "number" ? String(info.size) : "unknown";
	return `"${encodeURIComponent(String(path))}-${version}-${size}"`;
}
 ...
 */
export function fileStream(ctx, target, total, signal, start = 0, length) {
	let offset = start;
	const limit = typeof length === "number" ? start + length : undefined;
	return new ReadableStream({
		async pull(controller) {
			try {
				...
				const window = limit === undefined ? DEFAULT_WINDOW_BYTES : Math.min(DEFAULT_WINDOW_BYTES, limit - offset);
				const bytes = await ctx.fs.readByteRange(target, { offset, length: window }, signal);
				...
				if (limit !== undefined && offset >= limit) controller.close();
				else if (total !== undefined && offset >= total) controller.close();
			} catch (error) {
...
export async function downloadResponse(ctx, request) {
	...
	const etag = etagOf(resolved.path, info);
	const headers = {
		...
		"etag": etag,
		"accept-ranges": total === undefined ? "none" : "bytes"
	};
	// A resumed download is the browser's own retry carrying a Range header, so
	// honouring the range is the whole feature: the download manager does the rest.
	if (total !== undefined) {
		const ifRange = request.headers.get("if-range");
		const unchanged = ifRange === null || ifRange === etag || ifRange === `W/${etag}`;
		if (unchanged) {
			...
			if (range !== undefined) {
				...
				return new Response(fileStream(ctx, resolved.target, total, request.signal, range.start, length), { status: 206, headers: partial });
			}
		}
	}
	headers["content-length"] = total === undefined ? undefined : String(total);
 ...
 * @param seed - a previous result to continue from, so an entry can be checksummed window by window.
 * @returns The unsigned checksum of everything fed so far.
 */
...
/** Extensions whose contents are already compressed, so deflating them only burns CPU. */
const STORED_EXTENSIONS = new Set([
	".png", ".jpg", ".jpeg", ".gif", ".webp", ".avif", ".ico", ".bmp",
	".mp4", ".m4v", ".mov", ".webm", ".mkv", ".avi",
	".mp3", ".aac", ".m4a", ".ogg", ".opus", ".flac",
	".zip", ".gz", ".tgz", ".xz", ".7z", ".rar", ".br", ".zst", ".lz4",
	".docx", ".xlsx", ".pptx", ".odt", ".ods", ".odp", ".jar", ".war", ".apk",
	".pdf", ".woff2"
]);

/**
 * The ZIP method for one entry.
 *
 * A known-compressed format is stored outright. Anything else is probed with a
 * level-1 deflate of the entry's first window — a few milliseconds — so an
 * unknown binary format that does not compress is stored rather than spending
 * level 9 on data that would only grow. Without a sample the answer is deflate,
 * because that is the right guess for text.
 * @param name - the entry's path inside the archive.
 * @param sample - the entry's first window, when the caller has read one.
 * @returns 8 for deflate, 0 for store.
 */
export function zipMethodFor(name, sample) {
	const text = String(name);
	const dot = text.lastIndexOf(".");
	if (dot !== -1 && STORED_EXTENSIONS.has(text.slice(dot).toLowerCase())) return 0;
	if (sample !== undefined && sample.byteLength >= 4096) {
		const probe = deflateRawSync(sample, { level: 1 });
		if (probe.byteLength >= sample.byteLength * 0.98) return 0;
	}
	return 8;
}
 ...
 * @param stamp - the archive timestamp.
 * @param streaming - set the data-descriptor flag and zero the sizes, because the
 *   checksum and lengths are only known after the body has been streamed.
 * @returns The header record.
 */
function localHeader(entry, stamp, streaming = false) {
	const name = Buffer.from(entry.name, "utf8");
	const header = Buffer.alloc(30);
	header.writeUInt32LE(0x04034b50, 0);
	header.writeUInt16LE(20, 4);
	header.writeUInt16LE(0x0800 | (streaming ? 0x0008 : 0), 6);
	header.writeUInt16LE(entry.method, 8);
	header.writeUInt16LE(stamp.time, 10);
	header.writeUInt16LE(stamp.date, 12);
	header.writeUInt32LE(streaming ? 0 : entry.crc, 14);
	header.writeUInt32LE(streaming ? 0 : entry.compressedSize, 18);
	header.writeUInt32LE(streaming ? 0 : entry.size, 22);
...
/**
 * The data descriptor that closes a streamed entry, carrying the checksum and
 * both lengths the local header had to leave at zero.
 * @param entry - the completed entry.
 * @returns The descriptor record.
 */
function dataDescriptor(entry) {
	const record = Buffer.alloc(16);
	record.writeUInt32LE(0x08074b50, 0);
	record.writeUInt32LE(entry.crc, 4);
	record.writeUInt32LE(entry.compressedSize, 8);
	record.writeUInt32LE(entry.size, 12);
	return record;
}
 ...
 */
/**
 * Produce the archive as a chunk sequence: a local header per entry, the entry's
 * body, and a data descriptor closing it, then the central directory and its end
 * record.
 *
 * The method is chosen from a level-1 probe of the first window, then the file is
 * read window by window straight into a raw deflate stream, so the
 * archive's memory is one deflate window plus one read window regardless of how
 * large the file is; the checksum and both lengths ride the data descriptor
 * because the local header is written before the body exists. The central
 * directory still carries the real values, which is what every reader uses.
 * @param ctx - host context carrying the filesystem.
 * @param entries - entries from `collectArchiveEntries` or a selection.
 * @param signal - request cancellation.
 * @param options - optional deflate level override, used by tests.
 * @returns The archive chunks.
 */
export async function* archiveChunks(ctx, entries, signal, options = {}) {
	const stamp = dosTimestamp(new Date());
	const level = options.level ?? ZIP_LEVEL;
	const central = [];
	let offset = 0;
	for (const entry of entries) {
		...
		if (entry.type === "directory") {
			...
			const header = localHeader(record, stamp, false);
			offset += header.byteLength;
			central.push(centralHeader(record, stamp));
			yield header;
			continue;
		}
		// Peek the first window so the method can be measured rather than assumed,
		// then stream from that same window instead of reading it twice.
		const first = await ctx.fs.readByteRange(entry.target, { offset: 0, length: DEFAULT_WINDOW_BYTES }, signal);
		const method = zipMethodFor(entry.name, first);
		const record = { ...entry, method, crc: 0, size: 0, compressedSize: 0, offset };
		const header = localHeader(record, stamp, true);
		offset += header.byteLength;
		yield header;
		let crc = 0;
		let size = 0;
		...
		if (method === 8) {
			...
			const feeder = (async () => {
				...
				for (;;) {
					...
					crc = crc32(chunk, crc);
					if (!deflate.write(Buffer.from(chunk))) await once(deflate, "drain");
					chunk = await ctx.fs.readByteRange(entry.target, { offset: read, length: DEFAULT_WINDOW_BYTES }, signal);
				}
				deflate.end();
			})();
			try {
	...
	yield directory;
	yield endOfCentralDirectory(central.length, directory.byteLength, directoryOffset);
}

/**
 * Adapt an async iterator to a response body, pulling one chunk at a time so the
 * consumer's pace is the producer's pace.
 * @param iterator - the chunk iterator.
 * @returns The response body stream.
 */
export function streamFrom(iterator) {
	return new ReadableStream({
		async pull(controller) {
			try {
...
/**
 * Stream the archive as a response body.
 * @param ctx - host context carrying the filesystem.
 * @param entries - entries from `collectArchiveEntries` or a selection.
 * @param signal - request cancellation.
 * @returns The response body stream.
...
export function archiveStream(ctx, entries, signal) {
	return streamFrom(archiveChunks(ctx, entries, signal));
}
