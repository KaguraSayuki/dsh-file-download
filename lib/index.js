 ...
 *
 * Together they are the read side of a browser file manager: list any directory
 * the serving filesystem can read, stream one file, render a script-free HTML
 * listing for phones, and stream a directory — or a multi-select — as a ZIP.
 *
 * This is deliberately wider than the official `workspaceFiles` service, which
 * confines directory listing to the Session workspace. The Session still names the
 * default root, but an absolute path may leave it: a headless host reached from
 * another device has no way to hand over a file that lives next to the workspace,
 * and the deployment-level fence is the Web GUI's own authentication, not the
 * workspace boundary. Reads stay read-only and every route sits behind
 * Connection's Host/Origin and browser-session checks, so there is no
 * unauthenticated side door.
 *
 * All I/O goes through the composed `ctx.fs` service, so file identity,
 * regular-file checks, and symlink handling are the backend's own. Downloads and
 * archives are paged streams: a bounded window per pull keeps memory flat for a
 * large file, and an archive buffers one file at a time rather than the tree.
 *
...
/**
 * Register the authenticated list, download, browse, and archive routes for this
 * plugin's lifetime.
